...
if uploaded_file and st.button("Generate Quiz"):
    ...
    # Display generation summary with detailed info
    ...
    if quiz_type == "Cloze":
        if cloze_count == num_questions:
            ...
            st.error("❌ Could not generate any cloze questions from this content.")
    else:
        if mcq_count == num_questions:
            ...
            st.error("❌ Could not generate any MCQ questions from this content.")
...
if st.session_state.quiz:
    ...
    if quiz_type == "Cloze":
        ...
        cloze_questions = quiz.get("cloze", [])
        actual_questions = min(len(cloze_questions), num_questions)
        
        for i, q in enumerate(cloze_questions[:num_questions], start=1):
    ...
    # MCQ
        ...
        actual_questions = min(len(mcq_questions), num_questions)
        
        for i, q in enumerate(mcq_questions[:num_questions], start=1):
            st.markdown(f'<div class="quiz-card"><strong>Q{i}.</strong> {q["question"]}</div>', unsafe_allow_html=True)
...
def create_pdf(quiz_data, num_questions):
    ...
    # Only export what was actually generated
    cloze_questions = quiz_data.get("cloze", [])[:num_questions]
    mcq_questions = quiz_data.get("mcq", [])[:num_questions]
    
    if cloze_questions:
        pdf.cell(0, 8, f"Cloze Questions ({len(cloze_questions)} questions):", ln=True)
        pdf.ln(2)
    ...
    if mcq_questions:
        pdf.ln(5)
        pdf.cell(0, 8, f"MCQ Questions ({len(mcq_questions)} questions):", ln=True)
        pdf.ln(2)
...
def create_word(quiz_data, num_questions):
    ...
    if cloze_questions:
        doc.add_heading(f"Cloze Questions ({len(cloze_questions)} questions):", level=1)
        for i, q in enumerate(cloze_questions, start=1):
            doc.add_paragraph(sanitize_text(f"Q{i}. {q['question']}"))
    
    if mcq_questions:
        doc.add_heading(f"MCQ Questions ({len(mcq_questions)} questions):", level=1)
        for i, q in enumerate(mcq_questions, start=1):
            doc.add_paragraph(sanitize_text(f"Q{i}. {q['question']}"))
            for idx, opt in enumerate(q.get("options", []), start=1):
...
if st.session_state.quiz:
    ...
    # Show download buttons with actual question counts
    ...
    st.info(f"Export includes: {cloze_count} cloze questions, {mcq_count} MCQ questions")